...
addCapabilityToChannel() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.iot.net:7050 --cafile $ORDERER_CA
    res=$?
    set +x
  else
    set -x
    peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.iot.net:7050 --tls true --cafile $ORDERER_CA
    res=$?
    set +x
  fi
